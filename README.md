...
- 📊 **实时监控** - 服务器状态、CPU/内存/带宽 12小时历史图表
- 💾 **智能缓存** - 本地缓存服务器信息，离线也能查看
- ~~🌐 **域名全管理** - 域名注册、DNS 解析、DNSSEC、域名过户、免费二级域名~~
- ~~🔒 **SSL 证书** - 证书申请、管理、自动续期~~
- ~~🎫 **工单系统** - 工单提交、回复、评分、产品授权~~
- ~~📈 **财务管理** - 订单查询、发票开具、消费统计~~
- PS：由于安全策略和api限制，上述划线内容还未实现（
- 🌙 **主题切换** - 亮色/暗色/跟随系统 主题自由切换
- ✏️ **自定义名称** - 长按服务器卡片自定义名称，云端同步
- ☁️ **云端同步** - API Key、个性化设置多设备同步

### 💡 高级功能
- **RCS 云服务器** - 完整的云服务器生命周期管理
  - 防火墙规则、NAT 映射
  - 弹性云盘、IP 管理
...
- `user_profiles` - 用户配置信息（包含 API Key、个性化偏好设置）
- `server_aliases` - 服务器自定义别名
