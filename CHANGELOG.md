...
#### 🎨 个性化设置
- **设置持久化**：使用 SharedPreferences 保存主题和卡片样式，关闭应用后保持设置
- **云端同步**：个性化设置支持 Supabase 云端同步，多设备共享配置

#### 🛒 购买功能增强
- **独立IP选择**：RCS购买页面新增独立IP开关和数量选择器（1-5个）
- **动态价格**：通过 `/product/rcs/price` API实时获取独立IP价格
- **价格详情**：购买页面完整展示独立IP费用明细

#### 🎯 界面优化
- **仪表盘样式**：服务器卡片圆形仪表盘显示CPU/内存使用率
- **IP地址显示**：仪表盘样式服务器卡片显示IP地址
- **地区标签**：商店套餐卡片配置信息旁显示地区标签
- **地区前置**：套餐标题前显示地区信息（如"🇺🇸 美国洛杉矶 KVM 标配版"）

### 🐛 Bug修复
- **续费价格修复**：修复API返回单月价格的解析逻辑，正确计算多月总价
- **深色模式导航栏**：优化未选中图标颜色，深色模式下使用 `Colors.grey[400]` 提升可见性
- **DNS查询功能**：实现真实DNS查询，优化结果展示（A/AAAA记录标签）

### 📦 技术改进
- 使用 `CustomPaint` 绘制圆形进度仪表盘
- 独立IP价格支持API动态获取
- 改进价格计算逻辑，支持多维度费用叠加
